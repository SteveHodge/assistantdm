...
/* TODO:
 * DTT: should be able to apply POIs and masks to ImageOptionsPanel
 *
...
public class MapToolFrame extends JFrame {
	...
	public MapToolFrame() {
		this(new File("D:/DnDBooks/_Campaigns/Ptolus Madness Rising/Maps/Ptolus/Small.png"), null);
	}

	public MapToolFrame(URI uri, MapToolCallback callback) {
		this(new File(uri), callback);
	}

	public MapToolFrame(File file, MapToolCallback callback) {
		super("MapTool");

		this.callback = callback;
		lastDir = file;
		imageURI = lastDir.toURI();
		buildFrame(openImage(lastDir));
	}

	void buildFrame(BufferedImage img) {
		image = img;
	...
	private void updateGrid() {
		...
		if (w != 0 && h != 0) {
			...
			offsetLabel.setText(String.format("%.2f, %.2f", grid.getXOffset(), grid.getYOffset()));
		}
	...
	private JPanel makeControls() {
		...
		applyButton.addActionListener(e -> {
			if (callback != null) {
				callback.apply(new Rectangle2D.Double(grid.getXOffset(), grid.getYOffset(), grid.getGridWidth(), grid.getGridHeight()));
			}
		});
		...
		c.gridy++;
		pane.add(new JLabel("Offset:"), c);
		c.gridy++;
		pane.add(offsetLabel, c);
		c.gridy++;
